...
このUUIDはTime Machineボリュームの強固な曖昧さ除去を提供する目的で Zeroconfによって宣伝される。

ボリュームのCNID情報を格納するデータベーステーブルの名前として、MySQLおよびSQLite CNIDバックエンドでも使用されている。

> 【注記】 このファイルを軽率に編集すべきでないし、他のサーバにコピーすべきでもない。
